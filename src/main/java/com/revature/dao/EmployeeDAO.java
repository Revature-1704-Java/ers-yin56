...
public class EmployeeDAO implements EmployeeDAOInterface{
	...
	public int login(String username, String password){
		...
		try(Connection conn = ConnectionUtil.getConnection()) {
			String sql = "SELECT EMPLOYEEID, USERNAME, PASSWD FROM EMPLOYEEE WHERE USERNAME = \'" + username  + "\' AND PASSWD = \'" +   password + "\'";
			//System.out.println(sql);
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			int rid = 0;
	...
	public void submitForm(int id, double amount){
		PreparedStatement ps = null;
		try(Connection conn = ConnectionUtil.getConnection()) {
			String sql = "UPDATE REIMBURSEMENT SET TOTAL = " + amount + " WHERE RID = " + id;
			//System.out.println(sql);
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
